...
const doczip = "https://github.com/docker/docs/archive/refs/heads/main.zip"
...
const header = `// Code generated by lesiw.io/ctrctl. DO NOT EDIT.
package ctrctl
...
func run() error {
	docdir, err := os.MkdirTemp("", "ctrctl.*")
	if err != nil {
	...
	if err := fetchZip(doczip, docdir); err != nil {
	...
	cliDocsDir := filepath.Join(docdir, "docs-main", "data", "engine-cli")
...
func addCliDefinition(
	in []byte, out io.Writer, tmpl *template.Template,
) error {
...
func cmdFromDefinition(buf []byte) (*ctrCliCmd, error) {
	...
	if err := cmd.deduplicateArgs(); err != nil {
		return nil, fmt.Errorf("failed to deduplicate '%s' args: %w",
			cmd.Command, err)
	}
...
func (cmd *ctrCliCmd) deduplicateArgs() error {
	...
	for _, arg := range cmd.Arguments {
		...
		if seen {
			if cmd.Varg != nil {
				return fmt.Errorf("multiple vargs: %s, %s", cmd.Varg.Name,
					arg.Name)
			}
